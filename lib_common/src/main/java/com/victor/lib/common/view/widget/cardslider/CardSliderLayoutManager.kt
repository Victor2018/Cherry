 ...
 * -----------------------------------------------------------------
 * File: CardSliderLayoutManager
 * Author: Victor
 * Date: 2020/7/11 下午 06:22
 * Description: 
 * -----------------------------------------------------------------
 */
class CardSliderLayoutManager: RecyclerView.LayoutManager, ScrollVectorProvider {
    ...
    constructor(context: Context, attrs: AttributeSet?, defStyleAttr: Int) {
        ...
        val defaultCardWidth = (DEFAULT_CARD_WIDTH * density)
        val defaultActiveCardLeft =
            (DEFAULT_ACTIVE_CARD_LEFT_OFFSET * density)
        val defaultCardsGap: Float = DEFAULT_CARDS_GAP * density

        if (attrs == null) {
            initialize(defaultActiveCardLeft.toInt(), defaultCardWidth.toInt(), defaultCardsGap, null)
        } else {
            ...
            try {
                attrCardWidth =
                    a.getDimensionPixelSize(R.styleable.CardSlider_cardWidth,
                        defaultCardWidth.toInt()
                    )
                attrActiveCardLeft = a.getDimensionPixelSize(
                    R.styleable.CardSlider_activeCardLeftOffset,
                    defaultActiveCardLeft.toInt()
                )
            ...
            val viewUpdater: ViewUpdater = loadViewUpdater(context, viewUpdateClassName, attrs)!!
    ...
    override fun onLayoutChildren(recycler: Recycler?, state: RecyclerView.State) {
        ...
        var anchorPos: Int = getActiveCardPosition()
    ...
    override fun onItemsRemoved(recyclerView: RecyclerView, positionStart: Int, itemCount: Int) {
        ...
        if (positionStart + itemCount <= anchorPos) {
    ...
    fun getActiveCardLeft(): Int {
        return activeCardLeft
    }

    fun getActiveCardRight(): Int {
        return activeCardRight
    }

    fun getActiveCardCenter(): Int {
    ...
    private fun scrollRight(dx: Int): Int {
        ...
        val step: Int = activeCardLeft / LEFT_CARD_COUNT
    ...
    private fun fillLeft(anchorPos: Int, recycler: Recycler) {
        ...
        val layoutStep: Int = activeCardLeft / LEFT_CARD_COUNT
        var pos =
